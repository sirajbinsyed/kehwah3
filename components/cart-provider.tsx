...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const updateQty = (id: string, qty: number) => dispatch({ type: 'UPDATE_QTY', id, quantity: qty })
  const removeItem = (id: string) => dispatch({ type: 'REMOVE_ITEM', id })

  const getTotalCount = () => state.items.reduce((sum, item) => sum + item.quantity, 0)
  const getTotalPrice = () => state.items.reduce((sum, item) => sum + (item.price * item.quantity), 0)

  return (
    <CartContext.Provider value={{ state, addItem, updateQty, removeItem, getTotalCount, getTotalPrice }}>
      {children}
